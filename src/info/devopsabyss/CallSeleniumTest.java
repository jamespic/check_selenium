...
public class CallSeleniumTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Option optionWarnTimeout = new Option("w", "warningTimeout", true, "expected test execution time (including WebDriver init. Generates a warning on timeout");
        ...
        seTest.options.addOption(optionWarnTimeout);
        ...
        try {
            ...
            if (result.wasSuccessful()) {
                ...
                boolean warn = false;
                if (cmd.hasOption("w")) {
                    timeout = Long.parseLong(cmd.getOptionValue("w"));
                }
                if (timeout > 0L && runtime > timeout) {
                    warn = true;
                }
                if (warn) {
                    output = seTest.NAGIOS_TEXT_WARNING + " - " + cmd.getOptionValue("c") + " Tests passed, but took longer than " + timeout +"ms | ExecTime=" + result.getRunTime() + "ms";
                    nagios_rc = seTest.NAGIOS_RC_WARNING;
                } else {
